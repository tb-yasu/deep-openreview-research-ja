...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `--min-relevance-score` | 0.1 | 最小関連性スコア（0.0-1.0） |
| `--top-k` | 100 | LLM評価対象とする論文の上位件数（最終レポートにもこの数が含まれる） |
...
LLM評価を並列実行することで、大幅な処理時間の短縮を実現しています：

| 論文数 | 順次処理 | 並列処理（10並列） | 高速化 |
|--------|---------|-------------------|--------|
| 10論文 | 30秒 | 3秒 | **10倍** |
| 50論文 | 150秒（2.5分） | 15秒 | **10倍** |
| 100論文 | 300秒（5分） | 30秒 | **10倍** |

*※ 1論文あたり3秒と仮定（実際の処理時間はLLMのレスポンス速度に依存）

### 最適化の仕組み

- **asyncio + Semaphore**: 最大10件の同時リクエストで効率的に実行
- **レート制限対策**: OpenAI APIのレート制限を考慮した並列数設定
- **エラーハンドリング**: 一部の評価が失敗しても他の評価を継続
- **キャッシュ機構**: 同じ論文の再評価を回避してAPI呼び出しを削減

### APIコスト削減のヒント

1. **適切なモデルを使用**: デフォルトの`gpt-4o-mini`が品質とコストのバランスが良い（より高性能が必要なら`gpt-4o`や`gpt-5`）
